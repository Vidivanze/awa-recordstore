<template>
  <div class="dark:bg-gray-900">
    <div class="max-w-2xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
      <h1 class="mb-2 text-4xl dark:text-white text-left">Derniers arrivages</h1>
      <h2 class="mt-5 mb-5 text-3xl text-left dark:text-white">Jazz</h2>

      <div class="grid grid-cols-1 gap-y-10 sm:grid-cols-2 gap-x-6 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
        <template v-for="arrival in arrival_jazz" :key="arrival.id">
          <div class="group">
            <arrival-card 
              :id="arrival.id" 
              :name="arrival.name" 
    ...
    <div class="max-w-2xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:max-w-7xl lg:px-8">
      <h2 class="mt-4 mb-5 text-3xl text-left dark:text-white">Rap et HipHop</h2>

      <div class="grid grid-cols-1 gap-y-10 sm:grid-cols-2 gap-x-6 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
        <template v-for="arrival in arrival_rap" :key="arrival.id">
          <div class="group">
            <arrival-card 
              :id="arrival.id" 
              :name="arrival.name" 
